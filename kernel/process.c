...
int do_fork( process* parent)
{
  ...
  for( int i=0; i<parent->total_mapped_region; i++ ){
	...
	map_pages(child->pagetable, parent->mapped_info[i].va, PGSIZE * parent->mapped_info[i].npages, lookup_pa(parent->pagetable, parent->mapped_info[i].va), prot_to_type(PROT_READ | PROT_EXEC, 1));
